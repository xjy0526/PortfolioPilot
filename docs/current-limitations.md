...
Polymarket、Telegram、Parqet、Shadow Agent、Tech Radar 和 Trade Advisor 分别由
`ENABLE_POLYMARKET`、`ENABLE_TELEGRAM`、`ENABLE_PARQET`、`ENABLE_SHADOW_AGENT`、
...
- 原生前端与部分 Compatibility 接口仍依赖全局状态；核心组合路径已迁移，但完整模块归档尚未完成。兼容 URL 的存在不表示 `state.portfolio_data` 是正式事实源。
